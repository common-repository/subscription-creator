        ...
        if ($count > 0) {
			...
			if (count($result) > 0) {
				foreach ($result as $key => $val) {
					...
					$file_script = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/admin/partials/'.$info["tool"] .'/generator/script.php';
					if (file_exists ( $file_script )){
					$path_script = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/asset/'.$info["tool"] .'/js/script-'.$lastid.'.js';
					ob_start();
					include ($file_script);
				...
				$file_style = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/admin/partials/'.$info["tool"] .'/generator/style.php';
				if (file_exists ( $file_style )){
					$path_style = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/asset/'.$info["tool"] .'/css/style-'.$lastid.'.css';
					ob_start();
					include ($file_style);
    ...
    function updItem($tblname, $info) {
        ...
        if ($count > 0) {
			...
			foreach ($result as $key => $val) {
				...
				$file_script = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/admin/partials/'.$info["tool"] .'/generator/script.php';
				if (file_exists ( $file_script )){
					$path_script = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/asset/'.$info["tool"] .'/js/script-'.$id.'.js';
					ob_start();
					include ($file_script);
					$content_script = ob_get_contents();
					$packer = new JavaScriptPacker($content_script, 'Normal', true, false);
				...
				$file_style = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/admin/partials/'.$info["tool"] .'/generator/style.php';
				if (file_exists ( $file_style )){
					$path_style = WP_PLUGIN_DIR.'/'.$info["plugdir"] .'/asset/'.$info["tool"] .'/css/style-'.$id.'.css';
					ob_start();
					include ($file_style);
					$content_style = ob_get_contents();										
					ob_end_clean();
